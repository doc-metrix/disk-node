...
var // Expectation library:
	...
	METRICS = require( './../docs/disk.json' ),
...
describe( 'doc-metrix-disk', function tests() {
	'use strict';
	...
	describe( 'dexists', function tests() {
		...
		it( 'should provide a method to check the existence of a device having associated metric documentation', function test() {
		...
		it( 'should return false for a device which does not have associated metric documentation', function test() {
			assert.notOk( metrics.dexists( 'unknown-device-name' ) );
		});

		it( 'should return true for a device which has associated metric documentation', function test() {
			assert.ok( metrics.dexists( 'dm-0' ) );
		});

		it( 'should return true for a device which has associated metric documentation regardless of input name case', function test() {
			assert.ok( metrics.dexists( 'dM-0' ) );
		});
